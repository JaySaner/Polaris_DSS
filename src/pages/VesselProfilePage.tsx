...
export const VesselProfilePage: React.FC<VesselProfilePageProps> = ({
  ...
  return (
    <div id="vessel-profile-page" className="flex-1 bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-100 p-4 md:p-6 overflow-y-auto space-y-5 font-sans">
      {/* Header Banner */}
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 dark:border-slate-800 pb-4">
        ...
        {savedSuccess && (
          <div className="flex items-center gap-2 px-3.5 py-1.5 bg-emerald-100 text-emerald-800 dark:bg-emerald-950 dark:text-emerald-300 border border-emerald-300 dark:border-emerald-800 rounded-xl font-semibold text-xs shadow-sm">
      ...
      <form onSubmit={handleSave} className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4 rounded-2xl space-y-4 shadow-sm">
          <span className="font-semibold text-slate-800 dark:text-slate-100 text-xs uppercase tracking-wider block border-b border-slate-100 dark:border-slate-800 pb-2.5">
            Vessel Identity & Hull Dimensions
          </span>

          <div className="space-y-1.5">
            <label className="text-slate-600 dark:text-slate-300 block text-xs font-semibold">Vessel Name / Call Sign</label>
            <input
              type="text"
              value={formState.name}
              onChange={(e) => setFormState({ ...formState, name: e.target.value })}
              className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 text-slate-800 dark:text-slate-100 rounded-xl p-2.5 text-xs font-medium focus:border-blue-500 focus:outline-none transition"
            />
          </div>

          <div className="space-y-1.5">
            <label className="text-slate-600 dark:text-slate-300 block text-xs font-semibold">IMO Polar Class Rating</label>
            <select
              value={formState.iceClass}
              onChange={(e) => setFormState({ ...formState, iceClass: e.target.value })}
              className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 text-blue-600 dark:text-blue-400 font-semibold rounded-xl p-2.5 text-xs focus:border-blue-500 focus:outline-none transition"
            >
              {polarClasses.map((pc) => (
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              ...
              <input
                ...
                className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 text-slate-800 dark:text-slate-100 rounded-xl p-2.5 text-xs font-mono"
              />
            </div>
            <div className="space-y-1.5">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4 rounded-2xl space-y-4 shadow-sm flex flex-col justify-between">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 text-blue-600 dark:text-blue-400 font-bold font-mono rounded-xl p-2.5 text-xs"
                />
              </div>
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 text-slate-800 dark:text-slate-100 font-mono rounded-xl p-2.5 text-xs"
                />
              </div>
              <div className="space-y-1.5">
              ...
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 text-amber-600 dark:text-amber-400 font-bold font-mono rounded-xl p-2.5 text-xs"
                />
              </div>
            </div>
          ...
          <button
            ...
            className="w-full py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl text-xs uppercase tracking-wider transition-all shadow-md active:scale-98"
          >
